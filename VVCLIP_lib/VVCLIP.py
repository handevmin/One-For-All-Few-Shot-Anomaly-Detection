...
class Transformer(nn.Module):
    ...
    def VVCLIP_forward(self, x, out_layers, ffn):
        idx = 0
        out_tokens = []
        for r in self.resblocks:
            idx += 1
            ...
            if idx in out_layers:
                ...
                if isinstance(x, list):
                    ...
                    # out_tokens.append([x[1].clone(),x[0].clone()])
    ...
    def forward(self, x: torch.Tensor, out_layers = [6, 12, 18, 24], DPAM_layer = None, ffn = False):
        ...
        if not self.text_layer:
            ...
            if DPAM_layer is None:
                ...
                x, out_tokens = self.VVCLIP_forward(x, out_layers, ffn)
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        self.anomaly_pos = torch.cat([self.positional_embedding[0].unsqueeze(dim=0), self.positional_embedding[pos_idx,:]], dim=0).cuda()
        ...
        # self.bn_post = nn.BatchNorm1d(width)
        self.in_post = nn.InstanceNorm1d(width)
        # self.gn_post = nn.GroupNorm(16, 256)
    ...
    def forward(self, x: torch.Tensor, features_list, ori_patch = False, proj_use = True, DPAM_layer = None, ffn = False):
        ...
        # anomaly_x = x + self.anomaly_pos.to(x.dtype)
        ...
        # print(x.shape, pos.shape)
        # anomaly_x = self.ln_pre(anomaly_x)
        # anomaly_x = anomaly_x.permute(1, 0, 2)  # NLD -> LND
        # [anomaly_x, anomaly_x_ori], anomaly_patch_tokens = self.transformer(anomaly_x, features_list, DPAM_layer = DPAM_layer, ffn = ffn)
        

        if True:
            patch_token_list = []
            patch_token_memory = []
            anomaly_patch_token_list = []
            ...
            return x_ori[0, :, :] @ self.proj, patch_tokens, patch_token_memory#, anomaly_patch_tokens
